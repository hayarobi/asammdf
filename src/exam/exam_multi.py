...
def read_and_plot_multi(input_filenames):
    # MDF 파일을 읽어옵니다.
    # path = "MDF 파일을 저장한 경로"
    # for input_filename in input_filenames:
    #     raw_data = MDF(input_filename)
    raw_data = MDF.concatenate(input_filenames)
    databases = {
    ...
    signals_data_frame.to_csv(Path(input_filenames[0]).parent.joinpath(output_filename))
    exit()
...
if __name__ == "__main__":
    ...
    read_and_plot_multi(intput_file_name)
