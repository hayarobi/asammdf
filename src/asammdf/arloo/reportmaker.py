...
class ReportMaker:
    ...
    def make_report(self):
        graph_dataurl = self.get_plot_graph_dataurl()
        ci_dataurl = self.get_ci_iamge_dataurl()

        start_time = self.summary_data.get_start_time()
        if start_time is None:
            # FIXME 코드스멜 시그널 선택을 명시적으로 하지 않아도 시작과 종료 시각은 따로 있을 수 있다.
            start_time = self.summary_data.origin_time
        end_time = self.summary_data.get_end_time()
        if end_time is None:
            # FIXME 코드스멜 시그널 선택을 명시적으로 하지 않아도 시작과 종료 시각은 따로 있을 수 있다.
            end_time = self.summary_data.origin_time + timedelta(seconds=1000000)
        signal_summaries = self.get_signal_summaries(self.summary_data.start_delta
                                                     , self.summary_data.end_delta)

        dialog = ReportDialog(self.plot_widget, signal_summaries)
        result = dialog.exec()
        if result:
            report_data = dialog.report_data
            report_data.graph_image = graph_dataurl
