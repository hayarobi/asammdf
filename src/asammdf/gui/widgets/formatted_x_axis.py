...
class FormattedXAxis(pg.AxisItem):
    adjustXRange = QtCore.Signal(object, object)
    ...
    def __init__(self, positioner, *args, **kwargs):
        ...
        re = QRegularExpression("^[0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}$")
        self.validator = QRegularExpressionValidator(re)
        self.arloo_range = [None, None]
    ...
    def tickStrings(self, values, scale, spacing):
        ...
        if self.text_conversion and self.mode == "phys":
            ...
            if self.format == "phys":
                strns = super(FormattedXAxis, self).tickStrings(values, scale, spacing)
    ...
    def raiseContextMenu(self, ev):
        ...
        if self.origin is not None:
            ...
            time_in_cursor = self.origin + timedelta(seconds=pos.x())
            time_in_cursor_str = time_in_cursor.strftime("%H:%M:%S")
            menu.addAction(f"시작 시각을 {time_in_cursor_str}로 설정")
        ...
        if action is None:
            return
        elif action.text() in ["Apply new axis limits","표시 시간 범위 조정"]:
            lower_value, upper_value = self.arloo_range
            if not lower_value:
                lower_value = self.range[0]
            if not upper_value:
                upper_value = self.range[1]

            self.update_range(lower_value, upper_value)
            self.adjustXRange.emit(self.uuid, (lower_value, upper_value))
        elif action.text() == "Edit Y axis scaling":
            self.scale_editor_requested.emit(self.uuid)
        elif action.text().startswith("시작 시각을 "):
            lower_value, upper_value = self.arloo_range
            if upper_value is not None and pos.x() > upper_value:
